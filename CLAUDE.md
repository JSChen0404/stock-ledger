...
- **股利成本內扣（按日時間序）**：`buildHoldings` 把交易與股利合併成時間序事件一起跑——收到現金股利當下就壓低該檔持股成本，因此**賣出時的均價已是內扣後成本，已實現損益自然含股利**（賣掉那部分分到的股利落在已實現，剩餘股分到的留在未實現）。已無持股時收到的股利直接計入已實現。每檔同時記 `cost`（內扣後）與 `rawCost`（未扣）；持股卡的「原始成本」=rawCost、「已領股利」=rawCost−cost（僅剩餘股分攤的部分，非該檔一輩子總額）。同日股利排在交易前（除息日賣方仍領得到）。
- **配股**：除權日前持股 × 每股配股 ÷ 10 = 新增股數，存成 `kind:'stockdiv'` 的 0 成本 buy。
- `sharesHeldBefore(code, exDate)` 用**嚴格小於** `t.date < exDate`，確保同日除權息的新股不灌入當日基準。
- **年化報酬率＝XIRR（貨幣加權）**：`cashFlows(code)` 取該檔歷來現金流（買進為負含手續費、賣出為正扣費稅、現金股利為正；配股 price=0 不產生流），末端補上今日市值，交給 `xirr()` 用二分法解。**不可改回「總報酬率開 n 次方」的 CAGR**——分批買進時晚進場的資金會被當成從第一天就投入而高估/低估。持有未滿 30 天或無報價時顯示「—」（短期外推會失真）。此處分母是原始投入、股利算正流入，與持股成本的「股利內扣」是兩套獨立算法，不重複計算。
